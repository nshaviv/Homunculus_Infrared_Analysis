...
The unique intersecting long vector lines in Figure 1e locate the stellar cross. In Figure 3, central dark row and column projections locate the stellar marker at pixel $(344.5,350.5)$. The printed marker was removed with a compact mask and Telea inpainting while retaining the central region in the scientific footprint.
...
\begin{center}\includegraphics[width=0.72\textwidth]{outputs/diag_tick_residuals.png}\end{center}

\section{Morphology and Flux Calibration}
Grayscale samples normal to Figure 1e vector segments were grouped into the 22 published levels by contiguous dynamic programming with a monotonicity penalty. Red, luminance, and constrained RGB predictors were compared by leave-one-level-out cross-validation. RMSE values were 291.58, 336.84, and 291.58 Jy arcsec$^{-2}$, respectively; the optimal constrained weights were $[1,0,0]$. The physically appropriate red-only model was retained.

Contour red values and assigned flux levels were fitted with a pooled-adjacent-violators isotonic relation and piecewise-linear interpolation. Emission below 30 Jy arcsec$^{-2}$ is unsupported. Values above the highest effective anchor, 2823.43 Jy arcsec$^{-2}$, are held there and flagged as lower bounds. The 3200 Jy arcsec$^{-2}$ contour is therefore unavailable.
...
\begin{center}\includegraphics[width=\textwidth]{outputs/comparison_fig1e_reconstruction.png}\end{center}
...
s=x\sin(PA)+y\cos(PA).
\]
Native pixels are accumulated in 0.1 arcsec bins. With intensity $I_i$, pixel area $A_{\rm pix}$, and bin width $\Delta s$,
\[
P(s)=\frac{\sum_i I_i A_{\rm pix}}{\Delta s}, \qquad
\sum P(s)\Delta s=\sum I_iA_{\rm pix}.
\]
Thus $P$ has units Jy arcsec$^{-1}$. The integrated profile flux is 92928.68 Jy. It remains provisional because the source map is figure-derived and sub-30 Jy arcsec$^{-2}$ emission is unconstrained.

\begin{center}\includegraphics[width=0.95\textwidth]{outputs/i18_major_axis_profile.png}\end{center}
